...
DEFAULT_BUNDLE_PATH = ARTIFACTS_DIR / "sentiment_bundle.joblib"
DEFAULT_REPORT_PATH = ARTIFACTS_DIR / "sentiment_report.json"
...
def extract_top_terms(pipeline: Pipeline, top_n: int = 12) -> dict[str, list[dict[str, float]]]:
    ...
    if hasattr(model, "coef_"):
        ...
        negative_index = classes.index(NEGATIVE_LABEL)
        weights = model.feature_log_prob_[positive_index] - model.feature_log_prob_[negative_index]
    else:
        return {"positive_terms": [], "negative_terms": []}

    ranked = sorted(zip(feature_names, weights), key=lambda item: item[1])
    negative_terms = [{"term": term, "weight": float(weight)} for term, weight in ranked[:top_n]]
    positive_terms = [
        {"term": term, "weight": float(weight)}
        for term, weight in ranked[-top_n:][::-1]
    ]
...
def train(data_path: Path, bundle_path: Path, report_path: Path) -> dict[str, object]:
    ...
    if best_pipeline is None:
        raise RuntimeError("No sentiment candidate models were available.")
    ...
    report = {
        ...
        "top_terms": extract_top_terms(best_pipeline),
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train a movie review sentiment classifier.")
